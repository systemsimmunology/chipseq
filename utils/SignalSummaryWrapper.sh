#!/bin/bash
#
# Wrapper for running SignalSummary.sh 
# 
WRONGARGS=1
if [ $# != 4 ]
then
...
for (( i=0 ; i<nb ; i++ )); do
    bfile_i=${bfiles[$i]}
    echo "-----------Processing "$bed_dir$bfile_i" $featurefile $featureprefix--------------"
    "$UTILDIR"SignalSummary.sh $bed_dir$bfile_i $featurefile  $featureprefix
done
